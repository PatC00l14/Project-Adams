...
def save_data(f_name):
    ...
    for i in range(0, device.n_ridges):
        ...
        LinesY[i].Point1 = [dx * (i+0.5) , 0 , dz] ; LinesY[i].Point2 = [dx* (i+0.5) , dy, dz]
        ...
        if device.ext_sink_mat == 0:
            ...
            LinesZ[i].Point1 = [dx * (i+0.5) , 0 , 0] ; LinesZ[i].Point2 = [dx* (i+0.5) , 0, dz]      
        else:
            LinesZ = np.append(LinesZ , PlotOverLine(registrationName=f'PlotOverLine{i}', Input=solved_object))        
            LinesZ[i].Point1 = [dx * (i+0.5) , 0 , -device.ext_sink_dim[2]*10**-6] ; LinesZ[i].Point2 = [dx* (i+0.5) , 0, dz]  
